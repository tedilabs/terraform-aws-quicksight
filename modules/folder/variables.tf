...
variable "permissions" {
  ...
  (Optional) A list of resource permissions on the QuickSight folder. Maximum of 64 items. Each value of `permissions` as defined below.
    ...
    (Optional) `role` - A role of principal with a pre-defined set of permissions. Valid values are `OWNER` and `READER`. Conflicting with `actions`.
  ...
  validation {
    condition = alltrue([
      ...
      contains(["OWNER", "READER"], permission.role) || length(permission.actions) > 0
    ])
    error_message = "Valid values for `permission.role` are `OWNER` and `READER`. If `role` is not set, then `actions` must be specified."
  }
}
