...
| <a name="input_permissions"></a> [permissions](#input\_permissions) | (Optional) A list of resource permissions on the data source. Maximum of 64 items. Each `permission` block as defined below.<br/>    (Required) `actions` - Set of IAM actions to grant or revoke permissions on. Maximum of 16 items.<br/>    (Required) `principal` - The Amazon Resource Name (ARN) of the principal. | <pre>list(object({<br/>    actions   = set(string)<br/>    principal = string<br/>  }))</pre> | `[]` | no |
